...
validators =
  [ -- byr (Birth Year) - four digits; at least 1920 and at most 2002.
    ("byr", \i -> let year = read i in year >= 1920 && year <= 2001),
    -- iyr (Issue Year) - four digits; at least 2010 and at most 2020.
    ("iyr", \i -> let year = read i in year >= 2010 && year <= 2020),
    -- eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
    ("eyr", \i -> let year = read i in year >= 2020 && year <= 2030),
    -- hgt (Height) - a number followed by either cm or in:
    -- If cm, the number must be at least 150 and at most 193.
    -- If in, the number must be at least 59 and at most 76.
...
part2 :: [[Field]] -> String
part2 = (++) "Part 2: " <$> show . length . filter validateFields

main :: IO ()
main = interact (runParser (unlines . sequence [part1, part2]))
  where
    runParser f c =
      case parse parsePassports "" c of
        Right passports -> f passports
        Left e -> show e

parsePassports :: Parser [[Field]]
parsePassports = many parseFields

parseFields = (:) <$> parseField <*> manyTill parseField (try (() <$ string "\n\n" <|> eof))
